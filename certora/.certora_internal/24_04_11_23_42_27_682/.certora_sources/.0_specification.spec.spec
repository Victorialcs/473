...
// Define a ghost variable to track the sum of all balances
ghost mathint sum_of_balances = 0;

// Hook into the balanceOf mapping storage updates
hook Sstore balanceOf[address a] uint256 newValue (uint256 oldValue) {
    sum_of_balances = sum_of_balances + (newValue - oldValue);
}

/** `totalSupply()` should be equal to the sum of all balances */
