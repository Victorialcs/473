...
external balanceOf(address) returns (uint256) envfree;
external totalSupply() returns (uint256) envfree;
external transfer(address recipient, uint256 amount) returns (bool);
external approve(address spender, uint256 amount) returns (bool);
external allowance(address owner, address spender) returns (uint256) envfree;
